-- ============================================================
-- 007: 근로자 직무 컬럼 및 조직/직급/직무 옵션 설정화
-- ============================================================

ALTER TABLE workers
  ADD COLUMN IF NOT EXISTS job text;

CREATE INDEX IF NOT EXISTS workers_job_idx ON workers(job);
...
SET data =
  ...
  || jsonb_build_object(
    'departments',
    COALESCE(data->'departments', jsonb_build_array('관리부', '생산부', '영업부', '구매부')),
    'ranks',
    COALESCE(data->'ranks', jsonb_build_array('사원', '주임', '대리', '과장', '차장', '부장', '팀장', '실장', '이사', '상무', '전무', '부사장', '사장', '대표')),
    'jobs',
    COALESCE(data->'jobs', jsonb_build_array('관리', '사상', '용접', '취부', '절단', '도비'))
  )
WHERE category = 'workTime';
